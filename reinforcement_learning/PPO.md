...
2. 值得注意的是，在大语言模型中，状态 $s_t$ 就是已经生成的 token 序列，动作 $a_t$ 是生成下一个 token。当前 token 生成后，下一个状态 $ s_{t+1} $ 是已生成的序列加上新生成的 token。

3. $\prod_{t=1}^{T_n} P_{\theta}(a_n^t | s_n^t)$：表示轨迹$\tau^n$ 中所有步骤$t$上采取的动作$a_n^t$ 在状态$s_n^t$ 下的联合概率。
...
6. $\sum_{t=1}^{T_n} R(\tau^n) \nabla \log P_{\theta}(a_n^t | s_n^t)$：利用累积回报$R(\tau^n)$ 加权每一步的对数梯度，体现策略梯度方法中的优势估计。

7. $\sum_{t=1}^{T_n} R(\tau^n) \log P_{\theta}(a_n^t | s_n^t)$：省略梯度符号后的形式，通常用于描述带有加权对数概率的情况。
...
总的来说，修改后的公式是对未来奖励的折扣求和，这样当前动作的概率就不再只取决于当前的奖励，而且取决于未来的奖励，这样就可以减小回报的方差，使得训练更加稳定。

还有一种情况会影响我们算法的稳定性，那就是在好的局势下和坏的局势下。比如在好的局势下，不论你做什么动作，你都会得到正的回报，这样算法就会增加所有动作的概率。会使得reward大的动作的概率大一些，使得reward小的动作的概率小一些，但是这样会让训练很慢，也会不稳定。最好是能够让相对好的动作的概率增加，相对坏的动作的概率减小。
...
$$
\frac{1}{N} \sum_{n=1}^{N} \sum_{t=1}^{T_n} A_\theta^{\text{GAE}}(s_t, a) \nabla \log P_{\theta}(a_n^t \mid s_n^t)
$$
